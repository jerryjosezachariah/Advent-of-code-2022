package jerry.adventofcode.year2022.day16.two;
...
public class Main {
  ...
  public static void main(String[] args) throws Exception {
    ...
    long maxGain = 0;
    int maxBitMap = (int) Math.pow(2, index) - 1;
    for (int i = 0; i <= maxBitMap; ++i) {
      long gain =
          calculateMaxGain(26, nodesMap.get("AA"), i)
              + calculateMaxGain(26, nodesMap.get("AA"), getComplement(i, maxBitMap));
      if (maxGain < gain) {
        maxGain = gain;
      }
    }
  ...
  private static int getComplement(int bitMask, int maxBitMask) {
